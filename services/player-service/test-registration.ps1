...
Write-Host "Test 1: Registering 'Kylian Mbappe' (simple request)..." -ForegroundColor Yellow
Write-Host ""
...
try {
    $response = Invoke-WebRequest -Method POST -Uri "http://localhost:8084/api/players/register" `
        -ContentType "application/json" -Body $request1 -ErrorAction Stop
...
try {
    $response = Invoke-WebRequest -Method POST -Uri "http://localhost:8084/api/players/register" `
        -ContentType "application/json" -Body $request2 -ErrorAction Stop
...
# Test 3: Duplicate registration (should return ALREADY_EXISTS)
...
Write-Host "  2. AI searches and validates (DeepSeek R1)" -ForegroundColor White
...
Write-Host '  Body: { "playerName": "Your Player" }' -ForegroundColor Gray
Write-Host "================================================" -ForegroundColor Cyan
